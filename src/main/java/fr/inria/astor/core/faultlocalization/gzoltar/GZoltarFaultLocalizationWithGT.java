 ...
 *
 */
public class GZoltarFaultLocalizationWithGT implements FaultLocalizationStrategy {

	public static final String PACKAGE_SEPARATOR = "-";
	Logger logger = Logger.getLogger(GZoltarFaultLocalizationWithGT.class.getName());
	...
	protected FaultLocalizationResult searchSuspicious(String locationBytecode, List<String> testsToExecute,
		...
		CodeLineCollector.getClazzElements();
		List<Statement> temp = new ArrayList<>();
		for (Statement stmt: gz.getSuspiciousStatements()) {
			if (ConfigurationProperties.getPropertyBool("includeTestInSusp")) {
				temp.add(stmt);
				continue;
			}
			//compName-qualifiedName of the statement class, limit this to gt class
			String compName = stmt.getMethod().getParent().getLabel();
			CtClass clazz = CodeLineCollector.isGTClass(compName);
			if (clazz != null && CodeLineCollector.isInScope(stmt.getLineNumber())) {
				temp.add(stmt);
			}
		}

		for (Statement gzoltarStatement : temp) {
